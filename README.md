...
```bash
puppter-lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
puppter-lambda$ sam local start-api
puppter-lambda$ curl http://localhost:3000/
```
...
```bash
puppter-lambda$ sam logs -n HelloWorldFunction --stack-name puppter-lambda --tail
```
...
```bash
sam delete --stack-name puppter-lambda
```
